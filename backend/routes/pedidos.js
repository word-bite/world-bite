...
const enviarNotificacao = async (cliente, codigo, nomeRestaurante) => {
    ...
    try {
        ...
        // Enviar SMS se telefone estiver disponível
        if (cliente.telefone) {
            try {
                await smsService.enviarNotificacaoPedido(
                    cliente.telefone,
                    codigo,
                    nomeRestaurante
                );
                ...
                console.error(`❌ Erro ao enviar SMS: ${smsError.erro || smsError.message}`);
            }
        }
...
router.post('/:id/pronto', async (req, res) => {
    try {
        ...
        if (pedidoAtualizado.cliente.telefone && pedidoAtualizado.codigoRetirada) {
            try {
                await smsService.enviarNotificacaoPedidoPronto(
                    pedidoAtualizado.cliente.telefone,
                    pedidoAtualizado.codigoRetirada
                );
                console.log(`✅ SMS de pedido pronto enviado para ${pedidoAtualizado.cliente.telefone}`);
            } catch (smsError) {
                console.error(`❌ Erro ao enviar SMS de pedido pronto: ${smsError.erro || smsError.message}`);
            }
        }
